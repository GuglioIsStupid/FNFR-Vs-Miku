...
return {
	...
	load = function(self)
		...
		if (storyMode and song == 1) or not storyMode then
			graphics:mikuTransitionIn()
		end
	end,
	...
	update = function(self, dt)
		...
		--[[ if song == 1 and curBeat >= 432 and curBeat < 492 and camera.zooming then
			camera.zoom = camera.zoom + 0.015
			uiScale.zoom = uiScale.zoom + 0.03
		elseif song == 2 and camera.zooming and ((curBeat >= 116 and curBeat < 180) or (curBeat >= 244 and curBeat < 309)) then
			camera.zoom = camera.zoom + 0.020
			uiScale.zoom = uiScale.zoom + 0.05
		end
		...
		if beatHandler.onBeat() then
			local curBeat = beatHandler.getBeat()

			if curWeek == 1 and (song == 1 and curBeat >= 432 and curBeat < 492 and camera.zooming) then
				camera.zoom = camera.zoom + 0.015
				uiScale.zoom = uiScale.zoom + 0.03
			elseif curWeek == 2 and (song == 2 and camera.zooming and ((curBeat >= 116 and curBeat < 180) or (curBeat >= 244 and curBeat < 309))) then
				camera.zoom = camera.zoom + 0.020
				uiScale.zoom = uiScale.zoom + 0.05
			elseif curBeat % 4 == 0 then
				if camera.zooming and camera.zoom < 1.35 and not camera.locked then 
					camera.zoom = camera.zoom + 0.015 * camera.camBopIntensity
					uiScale.zoom = uiScale.zoom + 0.03 * camera.camBopIntensity
				end
			end
				...
				love.graphics.push()				
					love.graphics.push()
						for j = #enemyNotes[i], 1, -1 do
							if enemyNotes[i][j].y <= 560 and drawEnemyNotes then
